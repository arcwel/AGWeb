...
import { withHtmlSinksAllowed } from '@/trusted-html'
...
function MermaidBlock({ code }: { code: string }): React.JSX.Element {
  const [diagram, setDiagram] = useState<SVGSVGElement | null>(null)
  ...
  useEffect(() => {
    ...
    void import('mermaid').then(async ({ default: mermaid }) => {
      try {
        ...
        // Mermaid writes its markup into a scratch element as it renders — an
        // HTML sink this page refuses — so the door is opened for exactly this
        // render (trusted-html.ts). What comes back is parsed and scrubbed
        // into nodes (svg-node.ts); markup that is not one SVG is a failure
        // like any other, and shows the fence text instead.
        const { svg } = await withHtmlSinksAllowed(() =>
          mermaid.render(`agweb-mermaid-${mermaidSeq++}`, code)
        )
        if (cancelled) return
        const node = svgToSafeNode(svg)
        if (node) {
          setDiagram(node)
        } else {
          // Said out loud: a diagram that silently shows as its fence text
          // looks like a document that never had one.
          console.warn('WebDeck: a mermaid diagram did not parse as SVG and was shown as text')
          setFailed(true)
        }
      } catch (error) {
        if (cancelled) return
        console.warn('WebDeck: a mermaid diagram failed to render and was shown as text —', error)
        setFailed(true)
      }
    })
    return () => {
  ...
  // Inserted as a node, never as markup: an HTML string is a Trusted Types
  // sink on this page and is refused.
  useEffect(() => {
  ...
  return (
    <span className="mermaid-diagram" ref={hostRef}>
      {!diagram && <span className="text-xs text-slate-500">rendering diagram…</span>}
    </span>
  )
}
